...
class DocumentService:
    ...
    async def create_department_root(self, tenant_id: str, department_id: str) -> Optional[Dict[str, str]]:
        ...
        try:
            ...
            existing_root: Optional[DocumentFolder] = result.scalar_one_or_none()
            ...
            if existing_root:
                ...
                root_folder = DocumentFolder(
                    ...
                    folder_name=DocumentConstants.ROOT_FOLDER_NAME,
    ...
    async def create_folder(
        ...
        try:
            ...
            # Check for duplicate folder path in department
    ...
    async def get_folder_tree(self, department_id: str, folder_id: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """Get folder tree structure starting from folder_id (or root if None)"""
        try:
            if folder_id:
                # Get specific folder
                result = await self.db.execute(
    ...
    def _build_storage_key(self, tenant_id: str, department_id: str, folder_path: str, document_uuid: str, filename: str) -> str:
        """Build storage key for MinIO using document UUID and recursive folder path"""
    ...
    async def upload_document(
        ...
        try:
            ...
            storage_key = self._build_storage_key(tenant_id, department_id, folder_path, str(doc.id), file_name)
    ...
    async def upload_document_to_folder(
        ...
        try:
            ...
            # Rollback MinIO if storage_key exists
            if storage_key:
                try:
                    await minio_service.delete_object(bucket, storage_key)
                except Exception:
